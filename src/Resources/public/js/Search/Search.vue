...
export default {
  ...
  data() {
    return {
      ...
      tabs: [],
      settings: {},
      classDto: 'Evrinoma\\EximBundle\\Dto\\LogSearchDto',
      tabSelected: '',
      apiUrlSearch: window.location.origin + '/evrinoma/api/exim/log/search',
      apiUrlSettings: window.location.origin + '/evrinoma/api/exim/log/settings',
      apiUrlSettingsSave: window.location.origin + '/evrinoma/api/exim/log/settings/save',
    }
  },
  mounted() {
    this.doLoad();
    ...
    _getAddData() {
      ...
      this.settings.some(function (group) {
        group.items.some(function (value) {
          data.push({fileId: value.id, active: value.active});
        });
      });

      return {searchFiles: data, class: this.classDto};
    },
    doSave() {
      ...
      this.settings.some(function (value) {
        value.items.some(function (item) {
          if (item.active === 'a') {
            ...
            axios
                .get(self.apiUrlSearch, {params: {class: self.classDto, searchString: self.searchText, searchFiles: [item.data.name]}})
